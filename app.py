...
def home():
    return app.send_static_file('index.html') # serve the index.html file from the static folder
@app.route('/analyze', methods=['POST']) #handles queries to /analyze endpoint using POST method
def analyze_sector():
    try:
        news_query = "Donald Trump tech"
        news_text = get_news_articles(news_query) # fetch news articles related to Donald Trump and tech sector
        gemini_output = gemini_analysis(news_text, "Technology") # analyze the news articles for the technology sector

        parsed_output = json.loads(gemini_output) # parse the gemini output as JSON
        # generate audio for news summary using text-to-speech, to be done later
        response = {
            "sentiment": parsed_output.get("sentiment"),
            "shortTermOutlook": parsed_output.get("shortTermOutlook"),
            "longTermOutlook": parsed_output.get("longTermOutlook"),
            "stocksAffected": parsed_output.get("stocksAffected"),
            "newsSummary": parsed_output.get("newsSummary")
        }
        return jsonify(response), 200 # return the response as JSON with HTTP status code 200 (OK)
    except Exception as e:
        print(f"Error in /analyze endpoint: {e}")
        return jsonify({"error": str(e)}), 500 # return error message with HTTP status code 500 (Internal Server Error)
def home():
